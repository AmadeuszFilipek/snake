...
class TestSnake(unittest.TestCase):
   ...
   def test_tail_distance_closed(self):
      sn = deque()
      ...
      results = snake.get_snake_to_tail_distances(sn, self.grid)
      expected = [1, 1, 1, 1, 1, 1, 1, 1]
      self.assertEqual(results, expected)
   
   def test_tail_distance_closed_in_corner(self):
      sn = deque()
   ...
   def test_tail_distance_top_wall_1(self):
      sn = deque()
      sn.append(Point(x=0, y=0))
      sn.append(Point(x=0, y=1))
      results = snake.get_snake_to_tail_distances(sn, self.grid)
      expected = [np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, 1, np.inf]
      self.assertEqual(results, expected)

   def test_tail_distance_top_wall_2(self):
      sn = deque()
      sn.append(Point(x=0, y=1))
      sn.append(Point(x=0, y=0))
      results = snake.get_snake_to_tail_distances(sn, self.grid)
      expected = [np.inf, np.inf, 1, np.inf, np.inf, np.inf, np.inf, np.inf]
      self.assertEqual(results, expected)
   
   def test_tail_distance_case_1(self):
      sn = deque()
      tail = Point(x=3, y=3)
      sn.append(tail)
      sn.append(snake.move_point(sn[-1], 'left'))
      sn.append(snake.move_point(sn[-1], 'left'))
      sn.append(snake.move_point(sn[-1], 'left'))
      sn.append(snake.move_point(sn[-1], 'left'))
      sn.append(snake.move_point(sn[-1], 'left'))
      sn.append(snake.move_point(sn[-1], 'down'))
      sn.append(snake.move_point(sn[-1], 'down'))
      sn.append(snake.move_point(sn[-1], 'down'))
      sn.append(snake.move_point(sn[-1], 'down'))
      sn.append(snake.move_point(sn[-1], 'right'))
      sn.append(snake.move_point(sn[-1], 'right'))
      sn.append(snake.move_point(sn[-1], 'up'))
      sn.append(snake.move_point(sn[-1], 'up'))
      results = snake.get_snake_to_tail_distances(sn, self.grid)
      expected = [2, 2, np.inf, np.inf, 1, 2, 2, 2]
      self.assertEqual(results, expected)
